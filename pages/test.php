<?php
	...
	function print_issue_begin() {
		...
		echo '<td class="category" width="15%">', lang_get('plugin_traceability_test_id'), '</td>';
		echo '</tr>';
	 ...
	 * @author rcasteran
	 */
	function print_issue( $p_version_row, $p_issue_id, $p_issue_level = 0, $p_test_id_value_filter ) {
		...
		if(count($t_custom_fields) > 0) {		
			foreach ($t_custom_fields as $t_field_id) {
				if($t_field_id == plugin_config_get('test_id_var_idx')) {
		...
		if($t_test_id > PLUGIN_TRACEABILITY_VAR_IDX_NONE)
		{
		...
		if((($p_test_id_value_filter != '') && (strpos($t_test_id_value, $p_test_id_value_filter) !== FALSE)) ||
			($p_test_id_value_filter == ''))
		{			
			...
			echo '<td>', '<a href="'.plugin_page('test', false).'&project_id='.$t_project_id.'">'.string_display_line($t_project_name).'</a>', '</td>';
			echo '<td>', '<a href="'.plugin_page('test', false).'&version_id='.$t_version_id.'">'.string_display_line($t_version_name).'</a>', '</td>';
			echo '<td>', string_get_bug_view_link( $p_issue_id ), '</td>';
			
			if( $t_bug->handler_id != 0 ) {
				echo '<td>', '<a href="'.plugin_page('test', false).'&handler_id='.$t_bug->handler_id.'">'.string_display_line(user_get_name($t_bug->handler_id)).'</a>', '</td>';
			} else {
				echo '<td/>';
			}
			...
			if($t_test_id_value != PLUGIN_TRACEABILITY_VAR_STATUS_DEFAULT) {
				echo '<td>';
				$t_test_id_values = preg_split('#(?<!\\\)\\'.plugin_config_get( 'id_delimiter' ).'#', $t_test_id_value);	
				foreach ($t_test_id_values as $t_value) {
					echo '<a href="'.plugin_page('test', false).'&test_id='.$t_value.'">'.$t_value;
					echo '<br/>';					
				}
				echo '</td>';				
			}
			else {
				echo '<td>', $t_test_id_value, '</td>';
			}
			echo '</tr>';
		}
	} /* End of print_issue() */
	...
	# Retrieve test identifier (filtering purpose)		
	$f_test_id_value =  gpc_get_string( 'test_id', '' );
	...
	log_traceability_event('Test - project identifiers count: '.count($t_project_ids));
		...
		foreach( $t_version_rows as $t_version_row ) {
			...
			if($t_count_ids > 0)
			{
				for ( $j = 0; $j < $t_count_ids; $j++ ) {
					...
					print_issue( $t_version_row, $t_issue_set_id, $t_issue_set_level, $f_test_id_value );
				}
			}		
		}
		$t_project_index++;
